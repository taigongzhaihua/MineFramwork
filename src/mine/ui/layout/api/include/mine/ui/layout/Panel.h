/**
 * @file Panel.h
 * @brief Panel —— 布局面板基类，管理 FrameworkElement 子元素集合。
 *
 * Panel 继承自 FrameworkElement，维护一个 FrameworkElement* 子元素列表。
 ...
 *   - 子元素必须是 FrameworkElement 的实例（或子类）
 */
...
public:
     ...
     */
    void add_child(FrameworkElement* child);
     ...
     */
    void remove_child(FrameworkElement* child);
     ...
     * @return      对应子元素指针
     */
    [[nodiscard]] FrameworkElement* child_at(uint32_t index) const noexcept;
...
protected:
    /// 子元素指针列表（非拥有，裸指针）
    containers::SmallVector<FrameworkElement*, 8> children_;
};
