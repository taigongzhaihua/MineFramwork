...
public:
    ...
    /**
     * @brief 设置物理像素与逻辑像素的缩放比（DPI 缩放因子）。
     * @param scale  物理像素 / 逻辑像素，如 200% DPI 时传入 2.0f。
     *               取值须 > 0，默认为 1.0f（无缩放）。
     *
     * 当渲染目标以物理像素大小创建时（提升 HiDPI 渲染精度），
     * 必须在每次 render() 前通过此函数告知渲染器实际缩放比。
     * 渲染器内部用逻辑大小（物理大小 / scale）填写 ViewportCB，
     * 使得 Canvas 的逻辑像素坐标到 NDC 的映射保持正确，
     * 同时像素着色器中的 fwidth() 在物理像素精度下工作。
     */
